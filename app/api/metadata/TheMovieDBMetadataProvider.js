...
export default class TheMovieDbMetadataProvider implements MetadataProviderInterface {
  ...
  getMovies(page: number = 1) {
    return this.theMovieDb
               ...
               .then(({ data }) =>
                 data.results.map(movie => formatMetadata(movie, 'movies', this.imageUri, this.genres))
               );
  }
  ...
  getMovie(itemId: string) {
    return this.theMovieDb
               .get(`movie/${itemId}`)
               .then(({ data }) => formatMetadata(data, 'movies', this.imageUri, this.genres));
  }
...
function formatMetadata(item, type: string, imageUri: string, genres) {
  return {
    ...
    runtime      : item.runtime ||
      ...
      ? parseRuntimeMinutesToObject(type === 'movies' ? item.runtime : item.episode_run_time[0])
      : {
        full   : 'n/a',
        hours  : 'n/a',
        minutes: 'n/a'
